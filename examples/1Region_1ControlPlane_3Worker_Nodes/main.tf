...
module "cluster" {
  ...
  # ----------------
  ssh_keys = {
    "cloud-server" = file("~/.ssh/cloud_server.pub")
  ...
  additional_cloud_init = {
    timezone = "Europe/Warsaw" # See available time zones https://en.wikipedia.org/wiki/List_of_tz_database_time_zones#List
  }
  ...
  node_pools = {
    system = {
      ...
      type               = "cpx11" # See available types https://docs.hetzner.com/cloud/servers/overview#shared-vcpu
      count              = 1
      labels = {
        # "control-plane" = "yes"
      }
